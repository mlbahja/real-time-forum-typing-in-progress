...
func Validation(user models.User, flag bool) error {
	if flag {
		...
		if !printable(user.FirstName) {
			return errors.New("first name must be printable")
		}
		if !printable(user.LastName) {
			return errors.New("Last name must be printable")
		}
		if user.Gender == "" {
			return errors.New("Gender cannot be empty")
		}
		if !validAge(user.Age) {
			return errors.New("Age should valid .")
		}
	}
